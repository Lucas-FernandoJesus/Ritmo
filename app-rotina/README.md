...
Na terça, a atividade de fortalecimento em **Hoje** ou **Semana** abre o treino A; na quinta, abre o treino B. Os atalhos A/B em **Progresso** abrem a mesma tela. O botão de retorno e o Voltar do navegador devolvem à tela anterior. O endereço `?treino=A` ou `?treino=B` também permite abrir diretamente a semana atual do respectivo treino.

A tela segue a semana e o modo Normal, Reduzido ou Mínimo salvos no aparelho. Cada movimento mostra a prescrição do plano, um exemplo prático em português e um link individual que abre o YouTube em outra aba. Os vídeos podem estar em inglês e ilustram movimentos-base com amplitude, ritmo ou equipamentos diferentes; a variação aplicável está escrita no cartão. As instruções continuam disponíveis offline; os vídeos exigem conexão. Os links e as observações de correspondência ficam em `src/exercise-demos.ts`. Esse catálogo é independente da pesquisa de Muay Thai em `docs/muay-thai/`.
